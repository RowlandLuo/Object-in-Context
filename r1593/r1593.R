rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240808_r1593_different_0.csv")
SimilarLearn <- SimilarLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>135) %>%
...
p1circle <- p1 + geom_circle(aes(x0 = 497, y0 = 258, r = 65), color = "green")
p1circle <- p1circle + geom_circle(aes(x0 = 220, y0 = 478, r = 65), color = "green")
p1circle
radius <- 65
center_x1 <- 497
center_y1 <- 258
...
BaselineLearn <- read_csv("240808_r1593_1_baseline_learn.csv")
BaselineLearn <- BaselineLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>85) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
Baselinelearn <- BaselineLearn %>%
...
p2 <- ggplot(df2, aes(x = x2, y = y2)) +
  geom_point(alpha = 0.3)
p2
p2circle <- p2 + geom_circle(aes(x0 = 376, y0 = 170, r = 60), color = "purple")
p2circle <- p2circle + geom_circle(aes(x0 = 130, y0 = 398, r = 60), color = "purple")
p2circle
radius <- 60
center_x1 <- 376
center_y1 <- 170
...
SimilarTest <- read_csv("240808_r1593_different_2.csv")
...
p3 <- ggplot(df3, aes(x = x3, y = y3)) +
  geom_point(alpha = 0.3)
p3
p3circle <- p3 + geom_circle(aes(x0 = 497, y0 = 258, r = 65), color = "green")
p3circle <- p3circle + geom_circle(aes(x0 = 215, y0 = 478, r = 65), color = "purple") +
  ggtitle("baseline")
p3circle
radius <- 65
center_x3 <- 497
...
DifferentLearn <- read_csv("240808_r1593_3_similar_learn.csv")
DifferentLearn <- DifferentLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>58) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
DifferentLearn <- DifferentLearn %>%
...
BaselineLearn1 <- read_csv("240808_r1593_4_baseline_learn.csv")
BaselineLearn1 <- BaselineLearn1 %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>69) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit %>%
  filter(!Value.Item2 == TRUE)
...
DifferentTest <- read_csv("240808_r1593_5_similar_test.csv")
...
p6 <- ggplot(df6, aes(x = x6, y = y6)) +
  geom_point(alpha = 0.3)
p6
p6circle <- p6 + geom_circle(aes(x0 = 366, y0 = 175, r = 60), color = "red")
p6circle <- p6circle + geom_circle(aes(x0 = 130, y0 = 393, r = 60), color = "blue")
p6circle
radius <- 60
center_x6 <- 366
center_y6 <- 175
