rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240808_r1595_different_0.csv")
...
p1circle <- p1 + geom_circle(aes(x0 = 492, y0 = 263, r = 65), color = "green")
p1circle <- p1circle + geom_circle(aes(x0 = 210, y0 = 483, r = 65), color = "green")
p1circle
radius <- 65
center_x1 <- 492
center_y1 <- 263
...
BaselineLearn <- read_csv("240808_r1595_1_baseline_learn.csv")
...
p2 <- ggplot(df2, aes(x = x2, y = y2)) +
  geom_point(alpha = 0.3)
p2
p2circle <- p2 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "purple")
p2circle <- p2circle + geom_circle(aes(x0 = 135, y0 = 393, r = 60), color = "purple")
p2circle
radius <- 60
center_x1 <- 381
center_y1 <- 170
...
SimilarTest <- read_csv("240808_r1595_2_baseline_test.csv")
...
p3 <- ggplot(df3, aes(x = x3, y = y3)) +
  geom_point(alpha = 0.3)
p3
p3circle <- p3 + geom_circle(aes(x0 = 381, y0 = 175, r = 60), color = "green")
p3circle <- p3circle + geom_circle(aes(x0 = 135, y0 = 398, r = 60), color = "purple") +
  ggtitle("baseline")
p3circle
radius <- 60
center_x3 <- 381
...
DifferentLearn <- read_csv("240808_r1595_3_similar_learn.csv")
DifferentLearn <- DifferentLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>57) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
DifferentLearn <- DifferentLearn %>%
...
p4 <- ggplot(df4, aes(x = x4, y = y4)) +
  geom_point(alpha = 0.3)
p4
p4circle <- p4 + geom_circle(aes(x0 = 381, y0 = 165, r = 60), color = "blue")
...
BaselineLearn1 <- read_csv("240808_r1595_4_baseline_learn.csv")
...
p5 <- ggplot(df5, aes(x = x5, y = y5)) +
  geom_point(alpha = 0.3)
p5
p5circle <- p5 + geom_circle(aes(x0 = 381, y0 = 175, r = 60), color = "red")
p5circle <- p5circle + geom_circle(aes(x0 = 135, y0 = 388, r = 60), color = "red")
p5circle
radius <- 60
center_x5 <- 381
center_y5 <- 175
...
DifferentTest <- read_csv("240808_r1595_5_baseline_test.csv")
DifferentTest <- DifferentTest %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>78) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  filter(!Value.Item2 == TRUE) %>%
  na.omit
...
p6 <- ggplot(df6, aes(x = x6, y = y6)) +
  geom_point(alpha = 0.3)
p6
p6circle <- p6 + geom_circle(aes(x0 = 376, y0 = 170, r = 60), color = "red")
