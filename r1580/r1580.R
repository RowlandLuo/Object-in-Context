rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240725_r1580_similar_0.csv")
...
BaselineLearn <- read_csv("240725_r1580_baseline_1.csv")
...
SimilarTest <- read_csv("240725_r1580_baseline_2.csv")
...
DifferentLearn <- read_csv("240725_r1580_baseline_3.csv")
...
p4 <- ggplot(df4, aes(x = x4, y = y4)) +
  geom_point(alpha = 0.3)
p4
p4circle <- p4 + geom_circle(aes(x0 = 381, y0 = 173, r = 75), color = "green")
...
BaselineLearn1 <- read_csv("240725_r1580_different_4.csv")
BaselineLearn1 <- BaselineLearn1 %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>49) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit %>%
  filter(!Value.Item2 == TRUE)
...
DifferentTest <- read_csv("240725_r1580_different_5.csv")
DifferentTest <- DifferentTest %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>162) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  filter(!Value.Item2 == TRUE) %>%
  na.omit
...
p6 <- ggplot(df6, aes(x = x6, y = y6)) +
  geom_point(alpha = 0.3)
p6
p6circle <- p6 + geom_circle(aes(x0 = 497, y0 = 253, r = 75), color = "purple")
p6circle <- p6circle + geom_circle(aes(x0 = 220, y0 = 473, r = 75), color = "green")
p6circle
radius <- 75
center_x6 <- 497
center_y6 <- 253
