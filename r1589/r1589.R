rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240731_r1589_0_baseline_learn.csv")
...
p1circle <- p1 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "blue")
p1circle <- p1circle + geom_circle(aes(x0 = 140, y0 = 388, r = 60), color = "blue")
p1circle
radius <- 60
center_x1 <- 381
center_y1 <- 170
...
BaselineLearn <- read_csv("240731_r1589_1_similar_learn.csv")
BaselineLearn <- BaselineLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>89) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
Baselinelearn <- BaselineLearn %>%
...
p2 <- ggplot(df2, aes(x = x2, y = y2)) +
  geom_point(alpha = 0.3)
p2
p2circle <- p2 + geom_circle(aes(x0 = 381, y0 = 175, r = 60), color = "red")
p2circle <- p2circle + geom_circle(aes(x0 = 130, y0 = 398, r = 60), color = "red")
p2circle
radius <- 60
center_x1 <- 381
center_y1 <- 175
...
SimilarTest <- read_csv("240731_r1589_2_baseline_test.csv")
...
p3 <- ggplot(df3, aes(x = x3, y = y3)) +
  geom_point(alpha = 0.3)
p3
p3circle <- p3 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "blue")
p3circle <- p3circle + geom_circle(aes(x0 = 130, y0 = 393, r = 60), color = "red") +
  ggtitle("baseline")
p3circle
radius <- 60
center_x3 <- 381
...
DifferentLearn <- read_csv("240731_r1589_3_baseline_learn.csv")
...
p4 <- ggplot(df4, aes(x = x4, y = y4)) +
  geom_point(alpha = 0.3)
p4
p4circle <- p4 + geom_circle(aes(x0 = 391, y0 = 175, r = 60), color = "green")
p4circle <- p4circle + geom_circle(aes(x0 = 130, y0 = 398, r = 60), color = "green")
p4circle
radius <- 60
center_x4 <- 391
center_y4 <- 175
...
BaselineLearn1 <- read_csv("240731_r1589_different_4.csv")
BaselineLearn1 <- BaselineLearn1 %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>149) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit %>%
  filter(!Value.Item2 == TRUE)
...
p5 <- ggplot(df5, aes(x = x5, y = y5)) +
  geom_point(alpha = 0.3)
p5
p5circle <- p5 + geom_circle(aes(x0 = 502, y0 = 258, r = 65), color = "purple")
p5circle <- p5circle + geom_circle(aes(x0 = 230, y0 = 478, r = 65), color = "purple")
p5circle
radius <- 65
center_x5 <- 502
center_y5 <- 258
...
DifferentTest <- read_csv("240731_r1589_5_baseline_test.csv")
...
p6 <- ggplot(df6, aes(x = x6, y = y6)) +
  geom_point(alpha = 0.3)
p6
p6circle <- p6 + geom_circle(aes(x0 = 381, y0 = 175, r = 60), color = "green")
p6circle <- p6circle + geom_circle(aes(x0 = 130, y0 = 403, r = 60), color = "purple")
p6circle
radius <- 60
center_x6 <- 381
center_y6 <- 175
