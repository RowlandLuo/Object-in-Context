rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240809_r1598_0_similar_learn.csv")
SimilarLearn <- SimilarLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>52) %>%
...
p1circle <- p1 + geom_circle(aes(x0 = 386, y0 = 165, r = 70), color = "blue")
...
BaselineLearn <- read_csv("240809_r1598_1_baseline_learn.csv")
BaselineLearn <- BaselineLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>59) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
Baselinelearn <- BaselineLearn %>%
...
SimilarTest <- read_csv("240809_r1598_2_baseline_test.csv")
...
p3 <- ggplot(df3, aes(x = x3, y = y3)) +
  geom_point(alpha = 0.3)
p3
p3circle <- p3 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "red")
p3circle <- p3circle + geom_circle(aes(x0 = 130, y0 = 388, r = 60), color = "blue") +
  ggtitle("baseline")
p3circle
radius <- 60
center_x3 <- 381
...
DifferentLearn <- read_csv("240809_r1598_different_3.csv")
DifferentLearn <- DifferentLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>167) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
DifferentLearn <- DifferentLearn %>%
...
p4 <- ggplot(df4, aes(x = x4, y = y4)) +
  geom_point(alpha = 0.3)
p4
p4circle <- p4 + geom_circle(aes(x0 = 492, y0 = 258, r = 65), color = "purple")
p4circle <- p4circle + geom_circle(aes(x0 = 215, y0 = 483, r = 65), color = "purple")
p4circle
radius <- 65
center_x4 <- 492
center_y4 <- 258
...
BaselineLearn1 <- read_csv("240809_r1598_4_baseline_learn.csv")
...
p5 <- ggplot(df5, aes(x = x5, y = y5)) +
  geom_point(alpha = 0.3)
p5
p5circle <- p5 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "green")
p5circle <- p5circle + geom_circle(aes(x0 = 135, y0 = 388, r = 60), color = "green")
p5circle
radius <- 60
center_x5 <- 381
center_y5 <- 170
...
DifferentTest <- read_csv("240809_r1598_5_baseline_test.csv")
DifferentTest <- DifferentTest %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>73) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  filter(!Value.Item2 == TRUE) %>%
  na.omit
...
p6circle <- p6circle + geom_circle(aes(x0 = 130, y0 = 388, r = 60), color = "purple")
p6circle
radius <- 65
center_x6 <- 381
center_y6 <- 175
