rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240725_r1579_baseline_0.csv")
...
BaselineLearn <- read_csv("240725_r1579_similar_1.csv")
BaselineLearn <- BaselineLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>256) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
Baselinelearn <- BaselineLearn %>%
...
SimilarTest <- read_csv("240725_r1579_similar_2.csv")
...
DifferentLearn <- read_csv("240725_r1579_different_3.csv")
DifferentLearn <- DifferentLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>96) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
DifferentLearn <- DifferentLearn %>%
...
p4 <- ggplot(df4, aes(x = x4, y = y4)) +
  geom_point(alpha = 0.3)
p4
p4circle <- p4 + geom_circle(aes(x0 = 497, y0 = 253, r = 75), color = "green")
p4circle <- p4circle + geom_circle(aes(x0 = 220, y0 = 473, r = 75), color = "green")
p4circle
radius <- 75
center_x4 <- 497
center_y4 <- 253
...
BaselineLearn1 <- read_csv("240725_r1579_baseline_4.csv")
BaselineLearn1 <- BaselineLearn1 %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>51) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit %>%
  filter(!Value.Item2 == TRUE)
...
p5 <- ggplot(df5, aes(x = x5, y = y5)) +
  geom_point(alpha = 0.3)
p5
p5circle <- p5 + geom_circle(aes(x0 = 381, y0 = 173, r = 75), color = "purple")
p5circle <- p5circle + geom_circle(aes(x0 = 130, y0 = 393, r = 75), color = "purple")
p5circle
radius <- 75
center_x5 <- 381
center_y5 <- 173
...
DifferentTest <- read_csv("240725_r1579_baseline_5.csv")
