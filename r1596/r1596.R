rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240809_r1596_0_similar_learn.csv")
...
p1circle <- p1 + geom_circle(aes(x0 = 376, y0 = 170, r = 70), color = "red")
p1circle <- p1circle + geom_circle(aes(x0 = 135, y0 = 398, r = 70), color = "red")
p1circle
radius <- 70
center_x1 <- 376
center_y1 <- 170
...
BaselineLearn <- read_csv("240809_r1596_1_baseline_learn.csv")
...
p2 <- ggplot(df2, aes(x = x2, y = y2)) +
  geom_point(alpha = 0.3)
p2
p2circle <- p2 + geom_circle(aes(x0 = 381, y0 = 170, r = 65), color = "blue")
p2circle <- p2circle + geom_circle(aes(x0 = 125, y0 = 393, r = 65), color = "blue")
p2circle
radius <- 65
center_x1 <- 381
center_y1 <- 170
...
SimilarTest <- read_csv("240809_r1596_2_similar_test.csv")
...
p3 <- ggplot(df3, aes(x = x3, y = y3)) +
  geom_point(alpha = 0.3)
p3
p3circle <- p3 + geom_circle(aes(x0 = 381, y0 = 175, r = 65), color = "red")
p3circle <- p3circle + geom_circle(aes(x0 = 125, y0 = 393, r = 65), color = "blue") +
  ggtitle("baseline")
p3circle
radius <- 65
center_x3 <- 381
...
DifferentLearn <- read_csv("240809_r1596_different_3.csv")
DifferentLearn <- DifferentLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>330) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
DifferentLearn <- DifferentLearn %>%
...
p4 <- ggplot(df4, aes(x = x4, y = y4)) +
  geom_point(alpha = 0.3)
p4
p4circle <- p4 + geom_circle(aes(x0 = 492, y0 = 263, r = 65), color = "green")
p4circle <- p4circle + geom_circle(aes(x0 = 220, y0 = 473, r = 65), color = "green")
p4circle
radius <- 65
center_x4 <- 492
center_y4 <- 263
...
BaselineLearn1 <- read_csv("240809_r1596_4_baseline_learn.csv")
...
p5 <- ggplot(df5, aes(x = x5, y = y5)) +
  geom_point(alpha = 0.3)
p5
p5circle <- p5 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "purple")
p5circle <- p5circle + geom_circle(aes(x0 = 130, y0 = 388, r = 60), color = "purple")
p5circle
radius <- 60
center_x5 <- 381
center_y5 <- 170
...
DifferentTest <- read_csv("240809_r1596_different_5.csv")
