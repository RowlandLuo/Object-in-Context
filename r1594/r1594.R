rm(list = ls())
library(tidyverse)
SimilarLearn <- read_csv("240808_r1594_0_baseline_learn.csv")
SimilarLearn <- SimilarLearn %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>280) %>%
...
p1circle <- p1 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "green")
p1circle <- p1circle + geom_circle(aes(x0 = 135, y0 = 398, r = 60), color = "green")
p1circle
radius <- 60
center_x1 <- 381
center_y1 <- 170
...
BaselineLearn <- read_csv("240808_r1594_different_1.csv")
...
p2 <- ggplot(df2, aes(x = x2, y = y2)) +
  geom_point(alpha = 0.3)
p2
p2circle <- p2 + geom_circle(aes(x0 = 502, y0 = 268, r = 65), color = "purple")
p2circle <- p2circle + geom_circle(aes(x0 = 210, y0 = 488, r = 65), color = "purple")
p2circle
radius <- 65
center_x1 <- 502
center_y1 <- 268
...
SimilarTest <- read_csv("240808_r1594_different_2.csv")
SimilarTest <- SimilarTest %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>100) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  na.omit
SimilarTest <- SimilarTest %>%
...
p3 <- ggplot(df3, aes(x = x3, y = y3)) +
  geom_point(alpha = 0.3)
p3
p3circle <- p3 + geom_circle(aes(x0 = 497, y0 = 263, r = 65), color = "purple")
p3circle <- p3circle + geom_circle(aes(x0 = 215, y0 = 478, r = 65), color = "green") +
  ggtitle("baseline")
p3circle
radius <- 65
center_x3 <- 497
...
DifferentLearn <- read_csv("240808_r1594_3_baseline_learn.csv")
...
p4 <- ggplot(df4, aes(x = x4, y = y4)) +
  geom_point(alpha = 0.3)
p4
p4circle <- p4 + geom_circle(aes(x0 = 381, y0 = 165, r = 60), color = "red")
p4circle <- p4circle + geom_circle(aes(x0 = 130, y0 = 393, r = 60), color = "red")
p4circle
radius <- 60
center_x4 <- 381
center_y4 <- 165
...
BaselineLearn1 <- read_csv("240808_r1594_4_similar_learn.csv")
...
p5 <- ggplot(df5, aes(x = x5, y = y5)) +
  geom_point(alpha = 0.3)
p5
p5circle <- p5 + geom_circle(aes(x0 = 381, y0 = 170, r = 60), color = "blue")
p5circle <- p5circle + geom_circle(aes(x0 = 125, y0 = 393, r = 60), color = "blue")
p5circle
radius <- 60
center_x5 <- 381
center_y5 <- 170
...
DifferentTest <- read_csv("240808_r1594_5_similar_test.csv")
DifferentTest <- DifferentTest %>%
  select(!15:24) %>%
  filter(Value.Item1.Item1.Item2>104) %>%
  rename(pixelx = Value.Item1.Item1.Item3.X, pixely = Value.Item1.Item1.Item3.Y, TimePoint = Value.Item1.Item1.Item2) %>%
  select(!1, !14) %>%
  filter(!Value.Item2 == TRUE) %>%
  na.omit
...
p6 <- ggplot(df6, aes(x = x6, y = y6)) +
  geom_point(alpha = 0.3)
p6
p6circle <- p6 + geom_circle(aes(x0 = 376, y0 = 165, r = 60), color = "blue")
p6circle <- p6circle + geom_circle(aes(x0 = 135, y0 = 393, r = 60), color = "red")
p6circle
radius <- 60
center_x6 <- 376
center_y6 <- 165
